...
import { getFeaturedResearch, getMediaUrl } from '@/payload';
    ...
    <div className="bg-white rounded-lg overflow-hidden shadow-md h-full flex flex-col transition-transform hover:scale-[1.02] duration-300">
      {abstract ? (
        ...
        <div className="p-6 flex flex-col h-full">
          ...
          <h3 className="text-lg font-bold text-brandNavy mb-2 line-clamp-2">
            {title}
          </h3>
...
const ResearchSection = async () => {
  const researchData = await getFeaturedResearch(3);
  ...
  const fallbackResearch = [
    {
      ...
      image: "/LandingPage/Research1.png",
      slug: "biodiversity-targets",
    ...
    },
    {
      ...
      image: "/LandingPage/Research2.png",
      slug: "social-media-religious-identity",
    ...
    },
    {
      ...
      image: "/LandingPage/Research3.png",
      slug: "deradicalization-programs",
  ...
  const researchItems = researchData.length > 0 ? researchData : fallbackResearch;

  return (
    <section className="w-full bg-brandNavy py-8">
      <div className="flex flex-col items-center pt-8 pb-8">
        <h2 className="text-3xl md:text-4xl font-bold text-brandWhite text-center font-halyard">
          RESEARCH
        </h2>
      </div>
      ...
      <div className="w-full max-w-[1536px] mx-auto px-2 sm:px-3 md:px-4 pb-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-5 lg:gap-6">
          {researchItems.map((item: any, index: number) => {
            const imageUrl = item.image 
              ? (typeof item.image === 'string' ? item.image : getMediaUrl(item.image))
              : item.image || `/LandingPage/Research${index + 1}.png`;
            
            const href = item.slug ? `/research/${item.slug}` : `/research/research-${index + 1}`;
        ...
        <div className="flex justify-center md:justify-end mt-10">
          <Link 
            ...
            className="inline-flex items-center px-6 py-3 rounded-md bg-brandWhite text-brandNavy font-medium hover:bg-opacity-90 transition-colors duration-300 font-halyard"
          >
            View All Research
